...
const itemVariants = cva(
  "group/item text-sm flex w-full flex-wrap items-center rounded-2xl border transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors [a]:hover:bg-muted",
  {
    variants: {
      variant: {
  ...
  "gap-2 group-has-data-[slot=item-description]/item:translate-y-0.5 flex shrink-0 items-center justify-center group-has-data-[slot=item-description]/item:self-start [&_svg]:pointer-events-none",
  {
    variants: {
      variant: {
        ...
        image:
          "size-10 group-data-[size=sm]/item:size-8 group-data-[size=xs]/item:size-6 overflow-hidden rounded-xl group-data-[size=xs]/item:rounded-lg [&_img]:size-full [&_img]:object-cover",
      },
    },
    defaultVariants: {
      variant: "default",
...
function ItemDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-sm font-normal line-clamp-2 text-left text-muted-foreground [&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-primary",
        className
      )}
      {...props}
    />
