...
Evaluation order
inject runs before define , and both run after all other transforms. See the transformer overview for the full order.
## Define ​
"Define" feature provides a way to replace global variables with constant expressions. This feature is similar to Terser 's global_defs option and esbuild's define option .
```
// input
...
```
## Inject ​
"Inject" feature provides a way to replace global variables with an import from a module. This feature is similar to esbuild's inject option and @rollup/plugin-inject .
```
// input
