...
const [disabledItems, enabledItems] = Arr.partition(
  items,
  (item) => item.enabled
)
```
...
const positiveCounts = Arr.filter(
  Record.values(countsById),
  (count) => count > 0
)
...
const activeCount = Record.reduce(
  usersById,
  0,
  (sum, user) => sum + (user.active ? 1 : 0)
)

const hasAdmin = Record.some(
  usersById,
  (user, id) => id.startsWith("admin-") && user.role === "admin"
)
